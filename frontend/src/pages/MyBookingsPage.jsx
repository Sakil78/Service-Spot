...
export default function MyBookingsPage() {
  ...
  async function fetchBookings() {
    ...
    try {
      ...
      const mappedBookings = fetchedBookings.map(booking => ({
        ...
        // Review tracking - use hasReview from backend (accurate)
        reviewed: booking.hasReview || false, // ✅ Backend now provides this
      }));
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      ...
      // success — refresh from server to get accurate data
      await fetchBookings();
    } catch (err) {
  ...
  const submitReview = async (e) => {
    ...
    try {
      ...
      // Refresh bookings from server to get updated hasReview flag
      // This ensures the "Write Review" button disappears
      console.log('🔄 Refreshing bookings to update reviewed status...');
      await fetchBookings();
      console.log('✅ Bookings refreshed - review button should now be hidden');
