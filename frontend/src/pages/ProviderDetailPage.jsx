...
import { providerAPI, reviewAPI, serviceAPI, bookingAPI, locationAPI } from '../services/api';
  ...
  async function fetchProviderData() {
    ...
    try {
      ...
      if (user && user.role === 'CUSTOMER') {
        ...
        // Fetch distance from customer to provider
        console.log('Distance fetch check (ProviderDetail):', {
          hasUser: !!user,
          userRole: user?.role,
          providerId: id,
          willFetchDistance: !!(user && user.role === 'CUSTOMER')
        });

        try {
          console.log('🔍 Fetching distance to provider:', id);
          const locationRes = await locationAPI.getUserLocation(id);
          ...
          console.error('❌ Could not fetch provider distance:', err);
                    ...
                    {user && user.role === 'CUSTOMER' && providerDistance && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 rounded-lg border border-green-200">
                        ...
                        <span className="text-sm font-semibold text-green-700">
                          📍 {providerDistance} away from you
                        </span>
                      </div>
                    )}
                  </div>
