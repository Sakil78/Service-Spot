...
import { providerAPI, reviewAPI, serviceAPI, specificAvailabilityAPI, locationAPI } from '../services/api';
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled && !serviceData) {
          ...
          // Fetch provider distance if user is a customer
          // Use the provider ID from serviceData since provider state might not be set yet
          const providerIdForDistance = serviceData?.provider?.id || serviceData?.providerId;

          console.log('Distance fetch check:', {
            hasUser: !!user,
            userRole: user?.role,
            providerIdForDistance,
            serviceDataProvider: serviceData?.provider,
            willFetchDistance: !!(user && user.role === 'CUSTOMER' && providerIdForDistance)
          });

          if (user && user.role === 'CUSTOMER' && providerIdForDistance) {
            try {
              console.log('🔍 Fetching distance to provider:', providerIdForDistance);
              const locationRes = await locationAPI.getUserLocation(providerIdForDistance);
              console.log('📍 Distance API response:', locationRes.data);

              if (locationRes.data?.success && locationRes.data.data?.distanceFormatted) {
                if (!cancelled) {
                  setProviderDistance(locationRes.data.data.distanceFormatted);
                  console.log('✅ Provider distance set:', locationRes.data.data.distanceFormatted);
                }
              } else {
                console.warn('⚠️ No distance data in response:', locationRes.data);
              }
            } catch (distErr) {
              console.error('❌ Could not fetch provider distance:', distErr);
              console.error('Error details:', {
                message: distErr.message,
                response: distErr.response?.data,
                status: distErr.response?.status
              });
            }
          } else {
            console.log('❌ Distance fetch skipped - conditions not met');
          }
        }
      } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white shadow-lg border border-gray-200 p-6 rounded-2xl mb-8 grid md:grid-cols-3 gap-6 items-center">
        <div className="md:col-span-2">
          ...
          <p className="text-sm text-gray-600 mb-3">Trusted local professionals — verified & reviewed</p>
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-700">
            ...
            {/* Distance Display - Only for logged-in customers */}
            {user && user.role === 'CUSTOMER' && providerDistance && (
      ...
      <div className="grid md:grid-cols-3 gap-6">
                ...
                <div key={service.id} className="bg-white shadow-lg border border-gray-200 p-5 rounded-2xl hover:scale-[1.02] transition-transform">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="mt-3 flex gap-3 items-center text-sm text-gray-700">
                        <div className="flex items-center gap-2">
        ...
        <aside className="space-y-6">
          <div className="bg-white shadow-lg border border-gray-200 p-5 rounded-2xl">
            ...
            <p className="text-sm text-gray-700">Licensed & insured professionals with transparent pricing and verified reviews.</p>
      ...
      <Modal isOpen={contactModalOpen} onClose={() => setContactModalOpen(false)} title={`Contact ${provider.name}`} size="sm">
        <div className="space-y-3">
          <p className="text-sm text-gray-600">You can reach out via phone or email:</p>
