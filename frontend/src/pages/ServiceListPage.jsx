...
export default function ServiceListPage() {
  ...
  const [distances, setDistances] = useState({}); // Store distances by provider ID
  const [loadingDistances, setLoadingDistances] = useState(false); // Loading state for distances
  ...
  useEffect(() => {
    ...
    async function fetchServices() {
      ...
      try {
        ...
        //Client-side filtering(can be moved to backend for performance)
  ...
  // Fetch distances for providers (only if user is logged in as customer)
  useEffect(() => {
    if (!user || user.role !== 'CUSTOMER' || services.length === 0) {
      console.log('Distance fetch skipped:', {
        hasUser: !!user,
        userRole: user?.role,
    ...
    console.log('Fetching distances for', services.length, 'services');
    setLoadingDistances(true);

    const fetchDistances = async () => {
      const distancePromises = services.map(async (service) => {
        if (!service.provider?.id) {
          console.warn('Service has no provider ID:', service.id, service.title);
          return null;
        }

        try {
          console.log(`Fetching distance for provider ${service.provider.id} (${service.provider.name || 'Unknown'})`);
          const response = await locationAPI.getUserLocation(service.provider.id);
          console.log(`Distance API response for provider ${service.provider.id}:`, response.data);

          if (response.data?.success && response.data.data?.distanceFormatted) {
            console.log(`✅ Distance for provider ${service.provider.id}: ${response.data.data.distanceFormatted}`);
            return {
              providerId: service.provider.id,
              distance: response.data.data.distanceFormatted
            };
          } else {
            console.warn(`⚠️ No distance data for provider ${service.provider.id}:`, response.data);
          }
        } catch (err) {
          console.error(`❌ Failed to fetch distance for provider ${service.provider.id}:`, err);
          ...
          if (err.response?.status === 400) {
            console.warn(`⚠️ Provider ${service.provider.id} distance unavailable: ${err.response?.data?.message || 'Missing location data'}`);
          }
        }
        return null;
      });
      ...
      results.forEach(result => {
        if (result) {
          distanceMap[result.providerId] = result.distance;
          successCount++;
        }
      });

      console.log(`Distance fetch complete: ${successCount}/${services.length} successful`);
                    ...
                    <div>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {user && user.role === 'CUSTOMER' && service.provider?.id && (
                              ...
                              {loadingDistances ? (
                                ...
                                <>
                                  ...
                                  <span className="text-xs font-semibold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">
                                    📍 {distances[service.provider.id]} away
                                  </span>
                                </>
                              ) : null}
                            </div>
