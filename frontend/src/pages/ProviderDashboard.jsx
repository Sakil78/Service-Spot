...
import { bookingAPI, serviceAPI, specificAvailabilityAPI, customerAPI, locationAPI } from '../services/api';
import {
  Calendar,
  Clock,
  DollarSign,
...
function BookingItem({ booking, updatingId, onUpdate, onViewCustomer }) {
  const statusClass = STATUS_STYLES[booking.status] || 'bg-gray-200 text-gray-800';

  return (
    <article
      className="bg-ora shadow-lg border border-gray-200 p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-start"
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-600 mt-1">
              ...
              <button
                ...
                className="font-medium text-primary hover:text-primary-dark underline decoration-dotted hover:decoration-solid inline-flex items-center gap-1 transition-colors"
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-600">
          <div className="flex items-center gap-2">
            ...
            <span>{formatDate(booking.bookingDate || booking.slotStart)}</span>
          </div>

          <div className="flex items-center gap-2">
            <Clock size={16} className="text-primary" />
            <span>{formatTime(booking.bookingTime || booking.slotStart)}</span>
          </div>
...
const ProviderDashboard = () => {
  ...
  /* Handler to view customer profile */
  const handleViewCustomer = async (customerId) => {
    ...
    try {
      ...
      // Fetch distance to customer
      try {
        const locationResponse = await locationAPI.getUserLocation(customerId);
        ...
        console.error('Error fetching customer distance:', distError);
        // Distance is optional, don't fail if it's not available
      }
    } catch (error) {
      ...
      try {
        if (bookingAPI?.getByUser) {
          ...
          fetchedBookings = rawBookings.map(booking => ({
            ...
            // Store date and time separately for proper display
            bookingDate: booking.bookingDate,
            bookingTime: booking.bookingTime, // Keep original time "15:00:00"
            // Combine date and time for slotStart field (for date display)
  ...
  const handleUpdateBookingStatus = async (bookingId, newStatus) => {
    ...
    try {
      ...
      if (bookingAPI?.updateStatus) {
        console.log('🔵 Sending update:', { bookingId, newStatus });
        await bookingAPI.updateStatus(bookingId, newStatus);
        console.log('✅ Update successful');

        // Refresh data from server to get accurate status
        await fetchDashboardData();
        return; // Exit early since fetchDashboardData already updates state
      } else {
        // simulate latency
        await new Promise(r => setTimeout(r, 700));
      }

      // update stats locally (fallback if API not available)
    ...
    <div className="container mx-auto px-4 py-8 min-h-screen bg-gray-50">
        ...
        <section className="space-y-4">
          ...
          {bookings.length === 0 ? (
            ...
            <div className="grid gap-4">
              {bookings.map(b => (
                <BookingItem key={b.id} booking={b} updatingId={updating} onUpdate={handleUpdateBookingStatus} onViewCustomer={handleViewCustomer} />
              ))}
            </div>
          )}
        </section>
        ...
        <form onSubmit={handleServiceSubmit} className="space-y-4">
          ...
          <div>
            ...
            {serviceFormErrors.title && (
              <p className="text-red-600 text-sm mt-1">{serviceFormErrors.title}</p>
            )}
          </div>
          ...
          <div>
            ...
            {serviceFormErrors.description && (
              <p className="text-red-600 text-sm mt-1">{serviceFormErrors.description}</p>
            )}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {serviceFormErrors.category && (
                <p className="text-red-600 text-sm mt-1">{serviceFormErrors.category}</p>
              )}
            </div>
            ...
            <div>
              ...
              {serviceFormErrors.price && (
                <p className="text-red-600 text-sm mt-1">{serviceFormErrors.price}</p>
              )}
            </div>
          </div>
          ...
          {/* Duration */}
          <div>
            <label htmlFor="durationMinutes" className="block text-sm font-medium text-gray-900 mb-2">
              Duration (minutes) *
            </label>
            <input
              type="number"
            ...
            {serviceFormErrors.durationMinutes && (
              <p className="text-red-600 text-sm mt-1">{serviceFormErrors.durationMinutes}</p>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-white/10">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-gray-700 mt-1">Set specific dates and times when you're available for this service</p>
              </div>
              <button
                type="button"
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-3">
                        ...
                        {/* Max Bookings */}
                        <div>
                          <label className="block text-xs text-gray-700 mb-1">Max Bookings</label>
                          <input
                            type="number"
                      ...
                      <button
                        ...
                        className="mt-5 bg-red-500/20 hover:bg-red-500/30 text-red-600 p-2 rounded transition-colors"
      ...
      >
        {loadingCustomer ? (
          ...
          <div className="space-y-6 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-6 rounded-2xl">
            {/* Customer Basic Info */}
            <div className="bg-gradient-to-br from-primary/90 to-purple-600 p-6 rounded-xl shadow-lg">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-white text-primary rounded-full flex items-center justify-center text-2xl font-bold shadow-md">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gradient-to-br from-blue-100 to-blue-50 p-4 rounded-lg space-y-3 border border-blue-200">
                {selectedCustomer.email && (
                  <div className="flex items-center gap-3 text-sm">
                    <Mail size={16} className="text-blue-600" />
                    <span className="text-gray-800 font-medium">{selectedCustomer.email}</span>
                  </div>
                )}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gradient-to-br from-purple-100 to-purple-50 p-4 rounded-lg space-y-3 border border-purple-200">
                {selectedCustomer.city && (
                  <div className="text-sm">
                    <span className="text-gray-700">City: </span>
                    <span className="font-semibold text-gray-900">{selectedCustomer.city}</span>
                  </div>
                )}
                ...
                {selectedCustomer.state && (
                  <div className="text-sm">
                    ...
                    <span className="font-semibold text-gray-900">{selectedCustomer.state}</span>
                  </div>
                )}
                ...
                {selectedCustomer.pincode && (
                  <div className="text-sm">
                    ...
                    <span className="font-semibold text-gray-900">{selectedCustomer.pincode}</span>
                  </div>
                )}
                ...
                {customerDistance && (
                  <div className="mt-4 pt-4 border-t border-purple-200">
                    <div className="bg-gradient-to-r from-primary to-purple-600 text-white rounded-lg p-4 text-center shadow-md">
                      <div className="text-xs font-semibold mb-2 opacity-90">Distance from you</div>
                      <div className="text-3xl font-bold">
                        {customerDistance.distanceFormatted || `${customerDistance.distanceKm?.toFixed(1)} km`}
                      </div>
                    </div>
                  </div>
                )}
            ...
            {selectedCustomer.address && (
              <div className="space-y-3">
                ...
                <div className="bg-gradient-to-br from-pink-100 to-pink-50 p-4 rounded-lg text-sm text-gray-800 border border-pink-200 font-medium">
                  {selectedCustomer.address}
                </div>
              </div>
            )}
