  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-3xl mx-auto">
        <div className="card p-6 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">My Profile</h1>
              <p className="text-sm text-gray-600 mt-1">Manage your contact and address details</p>
            </div>
          ...
          <form onSubmit={handleSubmit} aria-label="Customer profile form" className="space-y-6">
            {/* Personal */}
            <section aria-labelledby="personal-heading" className="bg-gradient-to-br from-blue-100 to-blue-50 p-5 rounded-xl border border-blue-200">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-white text-xl font-bold shadow-md">
                  {user.name?.charAt(0)?.toUpperCase() ?? 'U'}
                </div>
                <div>
                  <h2 id="personal-heading" className="text-lg font-semibold text-gray-900">Personal Information</h2>
                  <p className="text-xs text-gray-600">Name and phone used for bookings</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                  ...
                  {isEditing ? (
                    <>
                      <input
                        ...
                        className={`input-field bg-white ${errors.name ? 'border-red-500' : ''}`}
                ...
                <div>
                  ...
                  {isEditing && <p className="text-xs text-gray-600 mt-1">Email cannot be changed here</p>}
                </div>
                  ...
                  {isEditing ? (
                    <>
                      <input
                        ...
                        value={form.phone}
                        onChange={handleChange}
                        className={`input-field bg-white ${errors.phone ? 'border-red-500' : ''}`}
            ...
            {/* Address */}
            <section aria-labelledby="address-heading" className="pt-4 bg-gradient-to-br from-purple-100 to-purple-50 p-5 rounded-xl border border-purple-200">
              <h2 id="address-heading" className="text-lg font-semibold text-gray-900 mb-3">Address</h2>

              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="doorNo" className="block text-sm font-medium text-gray-900 mb-1">Door / Flat No.</label>
                  {isEditing ? (
                    <>
                      <input id="doorNo" name="doorNo" value={form.doorNo} onChange={handleChange} className={`input-field bg-white ${errors.doorNo ? 'border-red-500' : ''}`} aria-invalid={!!errors.doorNo} />
                      {errors.doorNo && <p className="text-red-500 text-sm mt-1">{errors.doorNo}</p>}
                    </>
                  ) : (
                    <p className="text-gray-900 font-medium">{user.doorNo}</p>
                  )}
                </div>

                <div>
                  <label htmlFor="pincode" className="block text-sm font-medium text-gray-900 mb-1">Pincode</label>
                  {isEditing ? (
                    <>
                      <input id="pincode" name="pincode" value={form.pincode} onChange={handleChange} className={`input-field bg-white ${errors.pincode ? 'border-red-500' : ''}`} maxLength={6} aria-invalid={!!errors.pincode} />
                ...
                <div className="md:col-span-2">
                  <label htmlFor="addressLine" className="block text-sm font-medium text-gray-900 mb-1">Address Line</label>
                  {isEditing ? (
                    <>
                      <input id="addressLine" name="addressLine" value={form.addressLine} onChange={handleChange} className={`input-field bg-white ${errors.addressLine ? 'border-red-500' : ''}`} aria-invalid={!!errors.addressLine} />
                      {errors.addressLine && <p className="text-red-500 text-sm mt-1">{errors.addressLine}</p>}
                    </>
                  ) : (
                    <p className="text-gray-900 font-medium">{user.addressLine}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <>
                      <select id="city" name="city" value={form.city} onChange={handleChange} className={`input-field bg-white ${errors.city ? 'border-red-500' : ''}`}>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <>
                      <select id="state" name="state" value={form.state} onChange={handleChange} className={`input-field bg-white ${errors.state ? 'border-red-500' : ''}`}>
