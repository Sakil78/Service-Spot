...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<UserResponse>> getUserByEmail(@PathVariable String email) {
        UserResponse user = userService.getUserByEmailDTO(email);
    ...
    /**
     * Refresh coordinates for a specific user based on their pincode.
     * Useful for users who registered before automatic geocoding or when it failed.
     * Supports both GET and POST for easy browser access.
     *
     * @param id user ID
     * @return updated user profile with new coordinates
     */
    @RequestMapping(value = "/{id}/refresh-coordinates", method = {RequestMethod.GET, RequestMethod.POST})
    public ResponseEntity<ApiResponse<UserResponse>> refreshUserCoordinates(@PathVariable Long id) {
        UserResponse user = userService.refreshUserCoordinates(id);
        return ResponseEntity.ok(ApiResponse.success("Coordinates refreshed successfully", user));
    }

    /**
     * Batch refresh coordinates for all users with NULL coordinates.
     * Admin operation to fix existing users with missing location data.
     * Supports both GET and POST for easy browser access.
     *
     * @return count of users updated
     */
    @RequestMapping(value = "/admin/refresh-all-coordinates", method = {RequestMethod.GET, RequestMethod.POST})
    public ResponseEntity<ApiResponse<String>> refreshAllMissingCoordinates() {
        int count = userService.refreshAllMissingCoordinates();
        String message = String.format("Successfully refreshed coordinates for %d users", count);
